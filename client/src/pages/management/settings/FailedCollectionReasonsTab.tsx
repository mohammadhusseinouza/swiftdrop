...
import type { FailedCollectionReasonSummary } from '../../../services/domain.types';
...
/**
 * Failed Collection Reasons — a SEPARATE catalog from Failed Delivery Reasons
 * (Phase 11.17). Same UI pattern; the two datasets are never merged. Reasons a
 * collection driver picks when a parcel-collection attempt fails (sender → company).
 */

type Editing =
  | { mode: 'create' }
  | { mode: 'edit'; row: FailedCollectionReasonSummary }
  | null;
...
export function FailedCollectionReasonsTab() {
  const canManage = useHasPermission(PERMISSIONS.SETTINGS_MANAGE);
  const isDesktop = useMediaQuery(MD_QUERY);
  const query = useGetFailedCollectionReasonsQuery();
  const [editing, setEditing] = useState<Editing>(null);
  const [toggling, setToggling] =
    useState<FailedCollectionReasonSummary | null>(null);
  const [update, updateState] = useUpdateFailedCollectionReasonMutation();
  ...
  const doToggle = (row: FailedCollectionReasonSummary) =>
    void (async () => {
      setToggleError(null);
      try {
  ...
  const columns = [
    {
      id: 'name',
      header: 'Reason',
      cell: (r: FailedCollectionReasonSummary) => r.name,
    },
    {
      id: 'requiresNotes',
      header: 'Requires notes',
      cell: (r: FailedCollectionReasonSummary) => (
    ...
    },
    {
      ...
      cell: (r: FailedCollectionReasonSummary) => r.sortOrder,
    },
    ...(canManage
      ? [
          {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-ink-muted">
          Reasons a collection driver can pick when a parcel-collection attempt
          fails (sender → company). Separate from Failed Delivery Reasons.
          Whether a reason forces the driver to add notes is set per reason.
        </p>
        {canManage && (
      ...
      {rows.length === 0 ? (
        <EmptyState
          className="py-12"
          title="No failed collection reasons configured."
        />
      ) : isDesktop ? (
        <Card flush>
          <DataTable
            ...
            rows={rows}
            getRowId={(r) => r.id}
            caption="Failed collection reasons"
          />
        </Card>
      ) : (
      ...
      <ConfirmationModal
        ...
        description={
          <p>
            This reason will no longer be offered for new failed-collection
            attempts. Collection attempts that already reference it stay
            unchanged.
          </p>
        }
      ...
      <ConfirmationModal
        ...
        description={
          <p>This reason will be offered again on failed collections.</p>
        }
...
function ReasonDialog({
  ...
  const [create, createState] = useCreateFailedCollectionReasonMutation();
  const [update, updateState] = useUpdateFailedCollectionReasonMutation();
  ...
  return (
    <ReferenceDialog
      open={!!editing}
      title={isEdit ? `Edit "${row?.name}"` : 'Add failed collection reason'}
    ...
    >
      ...
      <SelectField
        ...
        hint="When Yes, the collection driver must add notes to submit this reason."
        options={[
